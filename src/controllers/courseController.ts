...
export const deleteCourse = async (req: AuthRequest, res: Response): Promise<void> => {
	try {
		...
		const teams = await Team.find({ courseId: course._id }, '_id');
		const teamIds = teams.map((t) => t._id);
		if (teamIds.length > 0) {
			await ActivityLog.deleteMany({ teamId: { $in: teamIds } });
...
export const deleteCourseMilestone = async (req: AuthRequest, res: Response): Promise<void> => {
	try {
		...
		course.milestones = course.milestones.filter((m) => m._id.toString() !== req.params.milestoneId) as typeof course.milestones;
		await course.save();

		const teams = await Team.find({ courseId: course._id, 'milestones.courseMilestoneId': req.params.milestoneId });
		await Promise.all(
			teams.map((team) => {
				team.milestones = team.milestones.filter((m) => m.courseMilestoneId?.toString() !== req.params.milestoneId) as typeof team.milestones;
				return team.save();
			}),
		);
